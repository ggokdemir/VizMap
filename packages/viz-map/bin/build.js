...
const runCommands = {
    win32: {
        ...
        link: () => shell.exec('mklink /D "%SPLUNK_HOME%\\etc\\apps\\viz-map" "%cd%\\demo\\splunk-app"'),
    },
    nix: {
        build: () => shell.exec('export NODE_ENV=production && ./node_modules/.bin/webpack --mode=production'),
        demo: () => shell.exec('./node_modules/.bin/webpack serve --config demo/webpack.standalone.config.js --port 8080'),
        link: () => shell.exec('ln -s $PWD/demo/splunk-app $SPLUNK_HOME/etc/apps/viz-map-demo-app'),
    },
};
